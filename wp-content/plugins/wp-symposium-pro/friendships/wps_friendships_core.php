	...
	if ($user_id != $current_user->ID):
		...
		if (!$friends['status']):
			...
			if ($post_id = wp_insert_post( $my_post )):
				...
				wps_pro_insert_alert('friendship', $subject, $content, $current_user->ID, $user->ID, '', $url, $msg, 'pending', 'Chosen not to receive friendship requests by email');
...
function wps_friends_accept() {
	...
	if ($member1 == $current_user->ID || $member2 == $current_user->ID):
		...
		// Add alert
		wps_pro_insert_alert('friendship', $subject, $content, $accepted->ID, $sent->ID, '', $url, $msg, 'pending', 'Chosen not to receive friendship acceptances by email');
