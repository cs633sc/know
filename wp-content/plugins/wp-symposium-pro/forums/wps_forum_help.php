	...
	if (isset($the_post['wpspro_quick_start']) && $the_post['wpspro_quick_start'] == 'forum'):
		...
		if (is_wp_error($new_term)):
			...
			$post_content = '['.WPS_PREFIX.'-forum-post slug="'.$slug.'"]['.WPS_PREFIX.'-forum-backto slug="'.$slug.'"]['.WPS_PREFIX.'-forum slug="'.$slug.'"]';
 			$post_content .= '['.WPS_PREFIX.'-forum-comment slug="'.$slug.'"]['.WPS_PREFIX.'-forum-backto slug="'.$slug.'"]';
  	...
  	echo '<div class="wps_admin_getting_started_content" id="wps_admin_getting_started_forum" style="display:'.$display.'">';
		...
		<tr class="form-field">
			...
			<td>
				...
				<br /><span class="description"><?php echo sprintf(__('Default number of days after no activity that a forum post will close automatically (blank for never). Can be overridden for individual forums via Edit on <a href="%s">Manage All Forums</a>.', WPS2_TEXT_DOMAIN), admin_url( 'admin.php?page=wpspro_forum_setup' ) ); ?></span>
			</td>
        </tr>
        <tr class="form-field">
			<td scope="row" valign="top">
