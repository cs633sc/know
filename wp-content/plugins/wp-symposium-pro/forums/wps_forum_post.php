			...
			if( $post_terms && !is_wp_error( $post_terms ) ):
				...
				if ($user_can_see || current_user_can('manage_options')):
					...
					if (user_can_see_post($current_user->ID, $post->ID)):
						...
						$sql = "SELECT COUNT(comment_ID) FROM ".$wpdb->prefix."comments WHERE comment_post_ID = %d AND comment_parent = 0 AND comment_approved = 1";
						...
						// Get count of comments on replies
						$sql = "SELECT COUNT(comment_ID) FROM ".$wpdb->prefix."comments WHERE comment_post_ID = %d AND comment_parent > 0 AND comment_approved = 1";
                        $num_comment_comments = $wpdb->get_var($wpdb->prepare($sql, $post->ID));
                                    ...
                                    $post_html .= '<div class="wps_forum_post_comment_content">';
                                        ...
                                        if ( ( ($user_can_edit_forum || $user_can_delete_forum) && !$timed_out) || current_user_can('manage_options') || $is_forum_admin):
                                            ...
                                            $post_html .= '<div class="wps_forum_settings_options">';
                                                ...
                                                if (($user_can_edit_forum || $is_forum_admin) && $timeout-$age >= 0) $post_html .= '<br />('.sprintf(__('lock in %d seconds', WPS2_TEXT_DOMAIN), ($timeout-$age)).')';
                                        ...
                                        // Topic
                                        $post_content = wps_formatted_content($post->post_content, true);
                                            ...
                                            $comment_html .= '<div class="wps_forum_post_comment_content">';
                                                ...
                                                if ( (($user_can_edit_comment || $user_can_delete_comment) && !$timed_out) || $is_forum_admin):
                                                    ...
                                                    $comment_html .= '<div class="wps_forum_settings_options">';
                                                        ...
                                                        if (($user_can_edit_comment || $is_forum_admin) && $timeout-$age >= 0) $comment_html .= '<br />('.sprintf(__('lock in %d seconds', WPS2_TEXT_DOMAIN), ($timeout-$age)).')';
                                                ...
                                                // Show the reply
                                                $comment_content = wps_formatted_content($comment->comment_content);
                                                                    ...
                                                                    $sub_comment_html .= '<div class="wps_forum_post_comment_content">';
                                                                        ...
                                                                        if ( ($user_can_edit_comment || $user_can_delete_comment) || $is_forum_admin ):
                                                                            ...
                                                                            $sub_comment_html .= '<div class="wps_forum_settings_options">';
                                                                                ...
                                                                                if (($user_can_edit_comment || $is_forum_admin) && $timeout-$age >= 0) $sub_comment_html .= '<br />('.sprintf(__('lock in %d seconds', WPS2_TEXT_DOMAIN), ($timeout-$age)).')';
                                                                        ...
                                                                        // Subcomment
                                                                        $sub_comment_content = wps_formatted_content($subcomment->comment_content);
