...
class StaffDirectoryPlugin extends StaffDirectory_GoldPlugin
{
	...
	{	
		...
		{
			global $CompanyD_MediaButton;
			$CompanyD_MediaButton = new Company_Directory_Gold_Plugins_Media_Button('Staff', 'id-alt');
			$CompanyD_MediaButton->add_button('Staff List', 'staff_list', 'gp_staff_list_widget', 'id-alt');
			$CompanyD_MediaButton->add_button('Single Staff Member', 'staff_member', 'gp_single_staff_widget', 'id-alt');
			$CompanyD_MediaButton->add_button('Search Staff Members', 'search_staff_members', 'gp_search_staff_widget', 'id-alt');
		}
	...
	function staff_list_shortcode($atts, $content = '')
	{
		...
		if ( !empty($atts['style']) && $atts['style'] == 'grid' ) {						
			$grid_defaults = array(
				'show_bio' => false,
				'show_phone' => false,
				'show_email' => false,
			...
			$defaults = array_merge($defaults, $grid_defaults);
		}
		else if ( !empty($atts['style']) && $atts['style'] == 'table' ) {				
			...
			$defaults = array_merge($defaults, $table_defaults);
		}
		...
		if( $atts['style'] == 'table' && empty($atts['columns']) ) {
			$atts['columns'] = $this->build_column_list_from_attributes($atts);
		}
		
		// add any missing columns that were specified with show_X,
		// but not included in the column list
		$atts['columns'] = $this->add_missing_column_attributes($atts['columns'], $atts);
	...
	function get_staff_list_classes($atts)
	{
		...
		return apply_filters('staff_directory_staff_list_classes', $classes, $atts);
	}
