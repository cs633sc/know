...
class GP_Single_Staff_Widget extends WP_Widget
{
	...
	function form($instance){
		...
		$staff_members = get_posts('post_type=staff-member&posts_per_page=-1&nopaging=true');
		...
		<div class="gp_widget_form_wrapper">
			...
			<p>
				<label for="<?php echo $this->get_field_id('id'); ?>">Staff Member to Display</label>
				<select id="<?php echo $this->get_field_id('id'); ?>" name="<?php echo $this->get_field_name('id'); ?>" data-shortcode-key="id">
				<?php if($staff_members): ?>
					<?php foreach ( $staff_members as $staff_member  ) : ?>
					<option value="<?php echo $staff_member->ID; ?>"  <?php if($id == $staff_member->ID): ?> selected="SELECTED" <?php endif; ?>><?php echo $staff_member->post_title; ?></option>
					<?php endforeach; ?>
				<?php endif;?>
				</select>
				...
				<p>
					<label for="<?php echo $this->get_field_id('show_bio'); ?>">
						...
						<input class="widefat" id="<?php echo $this->get_field_id('show_bio'); ?>" name="<?php echo $this->get_field_name('show_bio'); ?>" type="checkbox" value="1" <?php if($show_bio){ ?>checked="CHECKED"<?php } ?> data-shortcode-value-if-unchecked="0"/>
						Bio
					</label>
				</p>
				<p>
				...
				<p>
					<label for="<?php echo $this->get_field_id('show_address'); ?>">
						...
						<input class="widefat" id="<?php echo $this->get_field_id('show_address'); ?>" name="<?php echo $this->get_field_name('show_address'); ?>" type="checkbox" value="1" <?php if($show_address){ ?>checked="CHECKED"<?php } ?> data-shortcode-value-if-unchecked="0"/>
						Address
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id('show_website'); ?>">
						...
						<input class="widefat" id="<?php echo $this->get_field_id('show_website'); ?>" name="<?php echo $this->get_field_name('show_website'); ?>" type="checkbox" value="1" <?php if($show_website){ ?>checked="CHECKED"<?php } ?> data-shortcode-value-if-unchecked="0"/>
						Website
					</label>
				</p>			
			</fieldset>
	...
	function widget($args, $instance)
	{
		...
		echo apply_filters('single_staff_widget_html', $output);
